...
class MeanReversionStrategy:
    def __init__(self, moex_connector, nymex_connector, iqfeed_connector, db_handler):
        ...
        print(11)
        #self.iqfeed_connector.listener.callback = self.on_iqfeed_update # Set the IQFeed class's callback function to on_iqfeed_update method
        print(22)
...
# what happens when IB ticker gets updated - update current bid&ask, send queue for price updates, send events to orders and pose events
    ...
    '''
    def on_iqfeed_update(self, iqfeed_symbol, iqfeed_bid, iqfeed_ask, iqfeed_signal_time):
        # update current bid/ask (from iqfeed)
        symbol = self.symbols[self.iqfeed_connector.symbols.index(iqfeed_symbol)]
        self.iqfeed_bid[symbol] = iqfeed_bid
        self.iqfeed_ask[symbol] = iqfeed_ask
        print(f"{datetime.now()}: on_iqfeed_update: {symbol} Current bid: {self.iqfeed_bid[symbol]}, current ask: {self.iqfeed_ask[symbol]}")
        self.limit_orders_events[symbol].put_nowait(iqfeed_signal_time) # send event/signal to update open orders

        moex_symbol = self.moex_connector.symbols[self.iqfeed_connector.symbols.index(iqfeed_symbol)]
        self.nymex_to_moex_queue.put_nowait(moex_symbol) # puting in queue MOEX contract corresponding to the NYMEX contract, which was called back
    ...
    async def iqfeed_checker(self, symbol):
        ...
        while not self.should_stop[symbol].is_set():
            ...
            if (iqfeed_bid_new != self.iqfeed_bid[symbol]) or (iqfeed_ask_new != self.iqfeed_ask[symbol]):
                ...
                self.limit_orders_events[symbol].put_nowait(datetime.now()) # send event/signal to update open orders
    ...
    async def order_flow(self, symbol, spread_step, pose_step, number_of_steps, mid_spread, mid_pose):
        ...
        while not self.should_stop[symbol].is_set():
            if not self.should_stop[symbol].is_set():
                ...
                nymex_signal_time = await self.limit_orders_events[symbol].get()
                if nymex_signal_time is None:
                    nymex_signal_time = datetime.now()
                    signal_produced_by = "pose_checker"
                else:
                    signal_produced_by = "nymex_update"
                signal_received = datetime.now() - nymex_signal_time
            ...
            print(f"{datetime.now()}: orders loaded")
            self.moex_connector.load_positions([moex_symbol]) # TODO: start counting pose locally (crosscheck each X minutes Y times)
            ...
            print(f"{datetime.now()}: moex_orders copied. Now start copying ")
            if symbol == "gold1":
                ...
                print(f"{datetime.now()}: NYMEX ask/bid copied from IB")
            else:
                ib_ask = self.iqfeed_ask[symbol] # ask price from IQFeed
                ib_bid = self.iqfeed_bid[symbol] # bid price from IQFeed
                print(f"{datetime.now()}: NYMEX ask/bid copied from IQFeed")
                # IQFeed subscription?
            
            moex_pose = self.moex_connector.positions[moex_symbol]
            print(f"{datetime.now()}: moex_pose copied: {moex_pose}. Starting calculations")
            ...
            print(f"{datetime.now()}: searchsortedAsync completed")

            orders = self.target_orders[symbol].copy()
            #cond = orders["id"]
            print(f"{datetime.now()}: old target orders copied. Starting iterative orders calcs")
            ...
            print(f"{datetime.now()}: iterative orders done. Raw orders dataframe ready")
            orders = orders.sort_values(by=['price_open'], ascending=True)
            print(f"{datetime.now()}: sorting done")
            orders = orders[orders["volume_current"] != 0]
            print(f"{datetime.now()}: zero volume raws deleted")
            orders["symbol"] = moex_symbol
            print(f"{datetime.now()}: symbol column added")
            orders_capped = orders.copy()
            orders_capped["volume_current"] = np.minimum(orders_capped["volume_current"], self.moex_max_lots[symbol])
            print(f"{datetime.now()}: volumes capped at max_volume parameter")
            self.target_orders[symbol] = orders_capped.copy()
            print(f"{datetime.now()}: target orders saved to self.target_orders")
            orders_reset = orders_capped.reset_index(names="target_index")
            print(f"{datetime.now()}: save target_index values")
            ...
            #print(orders)
            # TODO: add handler for blank cancel and modify orders tables
            print(f"{datetime.now()}: start sending cancel tasks")
            cancel_tasks = [asyncio.create_task(self.cancel_task(ticket, moex_symbol, nymex_signal_time, signal_produced_by)) for ticket in orders_to_cancel]
            print(f"{datetime.now()}: cancel tasks started, starting modify_tasks")
            modify_tasks = [asyncio.create_task(self.modify_task(item["ticket"], item["price_open_y"], moex_symbol, nymex_signal_time, signal_produced_by)) for item in orders_to_modify]
            print(f"{datetime.now()}: modify tasks sent")
            await asyncio.gather(*(cancel_tasks + modify_tasks))
            print(f"{datetime.now()}: cancel and modify tasks awaited")
            
            if not orders_to_cancel:
                place_df = merged[merged['_merge'] == 'right_only']
                print(f"{datetime.now()}: place_df defined")
                orders_to_place = place_df["target_index"].to_list()
                print(f"{datetime.now()}: orders_to_place defined")
                #print(orders_to_place)
                new_tasks = [asyncio.create_task(self.new_task(i, moex_symbol, nymex_signal_time, signal_produced_by)) for i in orders_to_place]
                print(f"{datetime.now()}: new_task sent")
                await asyncio.gather(*new_tasks)
                print(f"{datetime.now()}: new_task awaited")
                print(f"{datetime.now()}: all orders completed, time since signal = {datetime.now() - nymex_signal_time}")
            ...
            '''
            print(f"{datetime.now()}: orders_flow completed, time since signal = {datetime.now() - nymex_signal_time}")
...
# command to start grid trading
    async def start_grid_trading(self, symbol, spread_step, pose_step, number_of_steps, mid_spread, mid_pose):
        ...
        await self.nymex_connector.subscribe_bid_ask(nymex_contract) # TODO: change to be in line with limit_grid strategy
...
# command to start limit trading
    async def start_limit_trading(self, symbol, spread_step, pose_step, number_of_steps, mid_spread, mid_pose):
        ...
        if symbol == "gold1":
            ...
            #
            self.iqfeed_bid[symbol], self.iqfeed_ask[symbol] = await self.nymex_connector.get_bid_ask(nymex_contract)
            print(f"{datetime.now()}: {self.iqfeed_bid[symbol]} {self.iqfeed_ask[symbol]}")
            self.iqfeed_connector.listener.current_bid[iqfeed_symbol], self.iqfeed_connector.listener.current_ask[iqfeed_symbol] = self.iqfeed_bid[symbol], self.iqfeed_ask[symbol]
            print(f"{datetime.now()}: {self.iqfeed_connector.listener.current_bid[iqfeed_symbol]} {self.iqfeed_connector.listener.current_ask[iqfeed_symbol]}")
            print(f"{datetime.now()}: Starting subscription from IQFeed.")
            # IQFeed subscription?
            self.iqfeed_connector.active_symbols[iqfeed_symbol] = False
            self.iqfeed_task[symbol] = asyncio.create_task(self.iqfeed_connector.subscribe(iqfeed_symbol)) #
            print(f"{datetime.now()}: starting loading current bid/ask from IBKR...")
            #print(f"{datetime.now()}: start waiting for iqfeed subscription...")
            #while self.iqfeed_connector.active_symbols[iqfeed_symbol] == False:
            #    await asyncio.sleep(0.5) # wait until bid /ask loaded
            print(f"{datetime.now()}: subscription to IQFeed {symbol} completed: Current bid: {self.iqfeed_bid[symbol]}, Current ask: {self.iqfeed_ask[symbol]}")
        ...
        self.limit_orders_events[symbol].put_nowait(None)
        print(f"{datetime.now()}: first limit_orders_event initiated")
        ...
        self.active_trades[symbol] = [
            ...
            asyncio.create_task(self.iqfeed_checker(symbol))
        ]
        if not self.scheduler_on:
            asyncio.create_task(self.scheduler())
        await asyncio.gather(*self.active_trades[symbol])
...
# stop all trading for specific symbol or all symbols at once
    async def stop_trading(self,symbol=None):
        if symbol is None:
            ...
            for symbol in self.iqfeed_task:
                iqfeed_symbol = self.iqfeed_connector.symbols[self.symbols.index(symbol)]
                self.iqfeed_connector.active_symbols[iqfeed_symbol] = False
                await asyncio.gather(*self.iqfeed_task[symbol_to_cancel])
    ...
    async def stop_symbol_trading(self,symbol):
        ...
        if symbol == "gold1":
            ...
            print(f"{datetime.now()}: NYMEX {symbol} unsubscribed")
        else:
            # IQFeed subscription?
            iqfeed_symbol = self.iqfeed_connector.symbols[self.symbols.index(symbol)]
            self.iqfeed_connector.unsubscribe(iqfeed_symbol)
            print(f"{datetime.now()}: IQFEED {symbol} unsubscribed")
